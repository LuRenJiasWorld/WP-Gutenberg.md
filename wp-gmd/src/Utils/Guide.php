...
class Guide {
	...
	public function __construct( $text_domain ) {
		add_action( 'admin_enqueue_scripts', array( $this, 'gmd_admin_scripts' ) );
		$this->text_domain = $text_domain;
	}

	// Each pointer has its own function responsible for putting appropriate JavaScript into footer
	public function gmd_pointer_footer_script() {
		// Build the main content of your pointer balloon in a variable
		$pointer_content = __( '<h3>Hello! Gutenberg.md</h3>', $this->text_domain ); // Title should be <h3> for proper formatting.
		$pointer_content .= '<p>' .__( 'Welcome to the new version of WordPress Gutenberg.md editor, please go to WP Gutenberg.md to configure related settings!', $this->text_domain ) . '</p>';
		// In JavaScript below:
		// 1. "#menu-plugins" needs to be the unique id of whatever DOM element in your HTML you want to attach your pointer balloon to.
		// 2. "gmd" needs to be the unique id, for internal use, of this pointer
		// 3. "position" --
	...
	public function gmd_admin_scripts() {
		...
		// check for dismissal of gmd settings menu pointer 'gmd'
		if ( ! in_array( 'gmd', $seen_it ) ) {
			// flip the flag enabling pointer scripts and styles to be added later
			$do_add_script = true;
			// hook to function that will output pointer script just for gmd
			add_action( 'admin_print_footer_scripts', array( $this, 'gmd_pointer_footer_script' ) );
		}
